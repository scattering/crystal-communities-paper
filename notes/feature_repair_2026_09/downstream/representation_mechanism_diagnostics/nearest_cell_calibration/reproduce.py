...
def main():
    ...
    manifest = {
        "diagnostic": "Historical p95 calibration within fixed full-map nearest-centroid cells",
        "scope": "All methods use exactly 150,247 shared dated ICSD IDs. At each T, cells use nearest full-map centroid assignments and saved distances of ICSD entries dated <=T. Score later common-ID ICSD and the external cohorts against those cell-distance p95 values. Fixed full-map centers, fitted representations and partitions retain future information; this is not cutoff-trained validation.",
    ...
    for method in METHODS:
        ...
        for cutoff in (1990, 2000, 2010):
            training = ref.loc[ref.year <= cutoff]
            ...
            ic = count(scored)
            summary = {"n_calibration_icsd": len(training), "n_later_icsd": len(later),
    ...
    lines = ["# Historical calibration of nearest-centroid cells", "", manifest["scope"], "",
             "No features, centroids, graph partitions or nearest-centroid assignments were recomputed. The p95 radius is calibrated from distances of historical members of each nearest-centroid cell, rather than members of its original graph community. Cells without historical members count as frontier. A singleton's radius equals its saved distance to the fixed center, which can be positive.", "",
