...
def main():
    ...
    original_labels = {method: table(ROOT / path, "icsd_id") for method, path in partition_paths.items()}
    ...
    for method in METHODS:
        for cutoff in (1990, 2000, 2010):
            ...
            for key in common:
                if years[key] <= cutoff and int(original_labels[method][key]["community"]) >= 0:
                    row = refs[method][key]
            ...
            historical_n = sum(year <= cutoff for year in years.values())
            assert historical_n == stats["n_historical_common_icsd_total"]
            assert historical_n - stats["n_calibration_icsd"] == stats["n_original_noise_excluded_from_calibration"]
