...
def build_coordinates(name, production, graphlet, externals):
    ...
    if spec["coordinates"] in ("crystalweave_pca", "cdf_pca"):
        ...
        icsd = pca.transform(standardized)
        record.update(n_zero_scale_columns=int(np.sum(scaler.var_ == 0)), pca_solver=pca._fit_svd_solver,
                      explained_variance_ratio_sum=float(pca.explained_variance_ratio_.sum()),
                      coordinate_convention="frozen_transform_for_all_populations",
                      pca_reference_retransform_max_abs_error=float(np.max(np.abs(pca.transform(standardized) - icsd))))
...
def main():
    ...
    unknown = set(only) - set(ABLATIONS)
    if unknown:
        raise ValueError(f"Unknown FA_ONLY maps: {sorted(unknown)}")
    ...
    nearest_rows = nearest_cell_rates(summaries, common)
