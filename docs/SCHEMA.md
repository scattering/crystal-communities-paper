...
The Zenodo deposit unpacks so its contents sit **directly under
`notes/`** in a repository checkout (i.e. `notes/features.npy`). Every
script's default input paths are repo-anchored to `notes/`.

```
notes/                                             # = unpacked Zenodo deposit
...
│   ├── split_2000/...
│   └── split_2010/...
├── graph_time_summary.json                       # Figure 1 (temporal cliff) input
├── node_temporal_events.csv                       # per-entry node-events (dashboard scorer)
├── temporal_null_runs/
...
## `graph_time_summary.json`

Per-decade graph-time evolution summary consumed by
`make_fig_temporal_cliff.py` to render **Figure 1** (the temporal
cliff). Produced by `scripts/icsd_graph_time_evolution.py`.

```json
{
  "n_points": 167500,
  "n_outliers": 8123,
  "outlier_ratio": 0.0485,
  "community_birth_year": { "<community id>": 1987, ... },
  "by_decade": {
    "1910s": {
      "n_total": 35,
      "n_outlier": 2,
      "n_cluster_birth_point": 27,
      "n_core_attachment": 16,
      "n_periphery_attachment": 17,
      "n_existing_cluster": 6,
      "n_same_community_attachment": 4,
      "n_cross_community_attachment": 0,
      "n_bridge_attachment": 0,
      "outlier_ratio": 0.0571,
      "cluster_birth_point_ratio": 0.7714,
      "existing_cluster_ratio": 0.1714,
      "same_community_attachment_ratio": 0.1143,
      "cross_community_attachment_ratio": 0.0,
      "bridge_attachment_ratio": 0.0,
      "core_attachment_ratio": 0.4571,
      "periphery_attachment_ratio": 0.4857
    },
    "1920s": { ... },
    ...
    "2010s": { ... },
...
`by_decade` keys are decade labels (`"1910s"` … `"2010s"`) plus an
`"unknown"` bucket for entries with no usable publication year. The
figure consumes the five `*_ratio` fields per decade
(`outlier_ratio`, `bridge_attachment_ratio`,
`cross_community_attachment_ratio`,
`same_community_attachment_ratio`, `cluster_birth_point_ratio`) as a
100%-stacked area; the `n_*` counts are the un-normalised event
tallies they derive from.

---

## `node_temporal_events.csv`

Per-ICSD-entry node-event table (167,500 rows). Produced by the
graph-time evolution pipeline. Consumed by the interactive dashboard
(`ICSD_NODE_EVENTS_PATH`) via `load_community_metadata` for the
per-community centroid-distance / core-threshold statistics behind the
structural-accessibility score; also the `--node-events` input to
`analyze_structural_accessibility.py` and `analyze_alab_validation.py`.

| column | type | description |
| ------ | ---- | ----------- |
| `icsd_id` | int | ICSD collection code. |
| `year` | int | First-report year. |
| `decade` | str | Decade bucket (e.g. `1990s`). |
| `community` | int | Louvain community label (`-1` = noise). |
| `event_type` | str | Discovery-event class (community birth, same-/cross-community attachment, bridge, outlier). |
| `n_active_neighbors` | int | Active graph neighbours at the entry's publication time. |
| `n_active_same_community_neighbors` | int | Of those, in the same community. |
| `n_active_other_communities` | int | Distinct other communities among active neighbours. |
| `is_bridge_attachment` | bool | Whether the entry bridges two previously separate communities. |
| `distance_to_centroid` | float | Distance to the assigned community centroid in the frozen PCA basis. |
| `core_threshold` | float | Per-community 95th-percentile within-community distance (in-basin cutoff). |
| `core_periphery` | str | Core vs periphery classification within the community. |
