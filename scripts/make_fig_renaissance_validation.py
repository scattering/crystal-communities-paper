#!/usr/bin/env python3
"""Combined Figure 2 (legacy producer for the original submission; superseded by
make_repaired_renaissance_figures.py): renaissance-community
...
# Documented-event identifications for the top-16 communities (from SI §S8.4).
# kind: "renaissance" = one of the nine textbook field-defining renaissances (teal);
#       "magnets" = rank-1 Sm-Fe-N permanent magnets (orange, also textbook);
#       "program" / "applied" / "support" = program-driven survey, applied-chemistry surge or
#       post-cuprate support chemistry (grey; real signals, not renaissances — SI §S8.4).
EVENT_IDS = {
    1: ("Sm-Fe-N magnets (Coey ~1990)", "magnets"),
    2: ("CMR manganites (Jin 1994)", "renaissance"),
    3: ("RE–light-metal binaries survey (program-driven)", "program"),
    4: ("Heavy-fermion 1:1:2 campaigns, post-Steglich 1979 (program-driven)", "program"),
    5: ("SOFC perovskite cathodes (mid-1990s)", "renaissance"),
    6: ("2:1:2 RE-In-TM prototype survey, Pöttgen era (program-driven)", "program"),
    7: ("Doped MgAl2O4 spinels (applied chemistry)", "applied"),
    8: ("Dilute magnetic semiconductors (Dietl 2000)", "renaissance"),
    9: ("MAX phases (Barsoum 1996+)", "renaissance"),
    10: ("RE-aluminate substrates, post high-Tc (support chemistry)", "support"),
    11: ("Double perovskites for spintronics (Kobayashi 1998)", "renaissance"),
    12: ("NaCoO2 thermoelectrics (Terasaki 1997)", "renaissance"),
    13: ("Corundum/ilmenite solid solutions (applied chemistry)", "applied"),
...
def main() -> int:
    ...
    gs = fig.add_gridspec(2, 2, height_ratios=[1.0, 1.15], hspace=0.45, wspace=0.18,
    ...
    ax_a.set_title(f"(a) YBa$_2$Cu$_3$O$_7$-type 123 cuprate community (n = 378)\n"
                    f"pre-1986 = {n_pre}, post-1986 = {n_post} entries; community born after 1986",
    ...
    ax_b.set_title(f"(b) CMR-manganite community (n = 571)\n"
                    f"pre-1994 = {n_pre}, post-1994 = {n_post} entries; {fold:.0f}× fold-change",
                    fontsize=9.5, fontweight="bold", pad=4)
    ...
    ax_c.set_title("(c) Top-16 communities by step-change score — teal: nine textbook field-defining renaissances; "
                    "orange: Sm–Fe–N permanent magnets (#1);\ngrey: program-driven, applied-chemistry or support-chemistry signals (SI §S8.4)",
                    fontsize=9.5, fontweight="bold", pad=4)
    ax_c.set_xlim(0, 165)
